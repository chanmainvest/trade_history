...
erDiagram
    ...
    instrument_identifier_lookups {
      INTEGER lookup_id PK
      TEXT identifier_type "fund_code"
      TEXT asset_type "mutual_fund"
      TEXT institution_code
      TEXT normalized_name
      TEXT display_name
      TEXT currency
      TEXT status "pending resolved ambiguous"
      TEXT resolved_symbol
      TEXT evidence_url
    }
...
5. **Reviewed identifier lookups** — mutual-fund names that need an
  initial fund-code lookup are recorded in `instrument_identifier_lookups`.
  Repair only rewrites them after a reviewed row is marked `resolved`; it
  does not hardcode CIBC fund codes.
6. **Synthetic symbol** — strip leading verbs (`BOUGHT`/`SOLD`/...) and
